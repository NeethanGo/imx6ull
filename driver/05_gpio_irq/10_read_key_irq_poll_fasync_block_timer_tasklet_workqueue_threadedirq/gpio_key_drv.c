...
static irqreturn_t gpio_key_threaded_irq_handler(int irq, void *dev_id)
{
	int val, key;
	struct gpio_key *gpio_key = dev_id;
	...
	printk("gpio_key_threaded_irq_handler key:%d, val:%d\n", gpio_key->gpio, val);
	printk("gpio_key_threaded_irq_handler the process is\"%s\"(pid %d)\n",current->comm, current->pid);
...
static int gpio_key_probe(struct platform_device *pdev)
{
	...
	for(i=0; i<count; i++){
		...
		// 使用中断线程化处理
		err = request_threaded_irq(gpio_keys_100ask[i].irq, gpio_key_isr, gpio_key_threaded_irq_handler,
					IRQF_TRIGGER_RISING, "100ask_gpio_key", &gpio_keys_100ask[i]);
	}
